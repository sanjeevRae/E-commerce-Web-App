          ...
          <div className="flex items-start gap-3 sm:gap-5">
            <div className="flex w-[64px] shrink-0 flex-col gap-3 overflow-y-auto sm:w-[88px] lg:w-[104px]">
              {images.map((image, index) => (
                <button
                  key={`${image}-${index}`}
                  onClick={() => setQuantity(quantity => quantity) || undefined}
                  className={`h-16 w-16 shrink-0 overflow-hidden rounded-lg border-2 bg-[#f4f4f4] sm:h-24 sm:w-24 sm:rounded-xl sm:border-4 lg:h-28 lg:w-[104px] ${image === images[0] && index === 0 ? '' : ''} ${product.image === image && index === 0 ? '' : ''} ${images[index] === image ? '' : ''}`}
                >
                ...
                <div className="absolute left-0 top-[calc(100%+0.75rem)] z-20 w-[min(42rem,calc(100vw-2rem))] rounded-3xl border border-[#ececec] bg-white p-5 shadow-[0_24px_60px_rgba(0,0,0,0.12)] sm:p-6">
                  <div className="grid gap-6 sm:grid-cols-2">
                    {categoryGroups.map(group => (
                      <div key={group.title}>
                        <h3 className="font-body mb-3 text-sm font-semibold uppercase tracking-[0.12em] text-[#111111]">
                          {group.title}
                        </h3>
