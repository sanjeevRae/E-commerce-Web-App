...
import { ChevronDown, Menu, Search, ShoppingBag, UserCircle, X } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <>
      {isHome && (
        <div className={`fixed left-0 right-0 top-0 z-50 bg-[#1d1d1d] py-2 text-center text-white transition-transform duration-300 ${isScrolled && !isMobileMenuOpen ? '-translate-y-full' : 'translate-y-0'}`}>
      ...
      <header className={`fixed left-0 right-0 z-40 bg-white/95 backdrop-blur ${isHome ? 'top-8 sm:top-9' : 'top-0'} border-b border-[#ececec]`}>
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <button onClick={() => router.push('/')} className="font-serif text-2xl font-semibold text-[#111111]">
            ChitraTech Shop
          </button>
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <button
              ...
              className="hidden items-center gap-1 p-1 text-xs font-medium text-[#111111] md:flex"
            >
              Cart <span>({totalItems})</span>
            </button>
            <button className="hidden text-[#111111] md:block" aria-label="Account">
              <UserCircle size={18} />
            </button>
      ...
      {isSearchOpen && (
        <div className="fixed inset-0 z-[60] bg-black/40 px-4 py-6 backdrop-blur-[2px] sm:px-6" onClick={closeSearch}>
          <div
            className="mx-auto mt-16 max-w-2xl overflow-hidden rounded-3xl bg-white shadow-[0_24px_80px_rgba(0,0,0,0.18)]"
