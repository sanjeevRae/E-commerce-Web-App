...
export default function FeaturedProducts() {
  return (
    <section id="featured" className="bg-white px-3 py-12 sm:px-4 md:px-6 lg:px-10">
      ...
      <div className="mb-6 grid grid-cols-2 gap-2 font-body sm:mb-9 sm:grid-cols-2 md:flex md:flex-wrap md:gap-2 md:overflow-visible">
        {filterCategories.map((category, index) => (
          <button key={category} className={`font-body w-full rounded-full border px-2.5 py-2 text-[10px] leading-none sm:text-[10px] md:w-auto md:px-5 md:text-xs ${index === 0 ? 'border-[#111111] bg-[#111111] text-white' : 'border-[#dedede] bg-white text-[#111111]'}`}>
            {category}
          </button>
        ))}
      </div>
      <div className="grid grid-cols-2 gap-x-2 gap-y-4 sm:gap-5 lg:grid-cols-4">
        {products.slice(0, 4).map(product => (
          <Link key={product.id} href={`/product/${product.id}`} className="group min-w-0 text-left font-body">
            ...
            <p className="mb-1 truncate font-body text-[8px] uppercase tracking-[0.06em] text-[#777777] sm:text-[11px] sm:tracking-[0.1em]">{product.category}</p>
            <h3 className="line-clamp-2 min-h-[30px] font-body text-[10px] font-medium leading-snug text-[#111111] sm:min-h-[42px] sm:text-sm">{product.name}</h3>
            <p className="mt-1 font-body text-[11px] font-semibold text-[#111111] sm:mt-3 sm:text-sm">Rs{product.price.toFixed(2)}</p>
          </Link>
        ))}
      </div>
