...
export default function Products() {
  ...
  const getMobileSlotStyle = (offset: number) => {
    const slots: Record<number, { left: string; opacity: number; scale: number; zIndex: number; pointerEvents: 'auto' | 'none' }> = {
      [-2]: { left: '-8%', opacity: 0, scale: 0.68, zIndex: 1, pointerEvents: 'none' },
      [-1]: { left: '15%', opacity: 0.34, scale: 0.78, zIndex: 4, pointerEvents: 'auto' },
      [0]: { left: '50%', opacity: 1, scale: 1, zIndex: 20, pointerEvents: 'none' },
      [1]: { left: '85%', opacity: 0.34, scale: 0.78, zIndex: 4, pointerEvents: 'auto' },
      [2]: { left: '108%', opacity: 0, scale: 0.68, zIndex: 1, pointerEvents: 'none' },
    };
    return slots[offset];
  };
  ...
  return (
    <section id="products" className="overflow-hidden bg-white px-4 py-16 text-center md:px-6 md:py-20 lg:px-10 lg:py-14">
      ...
      <div className="relative mx-auto h-[330px] w-full max-w-[430px] sm:h-[380px] lg:hidden">
        {products.map((product, index) => {
          ...
          return (
            <button
              ...
              className="absolute bottom-0 m-0 -translate-x-1/2 appearance-none rounded-none border-0 bg-transparent p-0 leading-none outline-none transition-[left,opacity,transform] duration-700 focus:outline-none focus-visible:outline-none"
            ...
            >
              <img
                src={product.image}
                alt={product.name}
                className={`object-contain object-bottom transition-[height,width,filter,transform] duration-700 ${activeCard ? 'h-[320px] w-[54vw] max-w-[205px] sm:h-[365px] sm:max-w-[225px]' : 'h-[205px] w-[30vw] max-w-[112px] sm:h-[240px] sm:max-w-[125px]'}`}
