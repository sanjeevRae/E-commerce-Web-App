...
export default function Products() {
  ...
  const getMobileSlotStyle = (offset: number) => {
    const slots: Record<number, { left: string; opacity: number; scale: number; zIndex: number; pointerEvents: 'auto' | 'none' }> = {
      ...
      [-1]: { left: '13%', opacity: 0.24, scale: 0.76, zIndex: 4, pointerEvents: 'auto' },
      [0]: { left: '52%', opacity: 1, scale: 1, zIndex: 20, pointerEvents: 'none' },
      [1]: { left: '87%', opacity: 0.24, scale: 0.76, zIndex: 4, pointerEvents: 'auto' },
  ...
  return (
    <section id="products" className="overflow-hidden bg-white px-4 py-16 text-center md:px-6 md:py-20 lg:px-10 lg:py-14">
      ...
      <div className="relative left-1/2 h-[410px] w-screen -translate-x-1/2 lg:hidden">
          ...
          return (
            ...
            >
              <img
                src={product.image}
                alt={product.name}
                className={`object-contain object-bottom transition-[height,width,filter,transform] duration-700 ${activeCard ? 'h-[365px] w-[225px]' : 'h-[235px] w-[120px]'}`}
      ...
      <div className="mx-auto max-w-[300px] border border-[#d8d8d8] bg-white p-3 text-left shadow-sm lg:hidden">
      ...
      <div className="relative left-1/2 hidden min-h-[455px] w-screen -translate-x-1/2 lg:block">
        ...
        <div className="absolute left-[calc(50%+112px)] top-[178px] z-30 w-[190px] border border-[#d8d8d8] bg-white p-3 text-left shadow-sm xl:left-[calc(50%+126px)]">
