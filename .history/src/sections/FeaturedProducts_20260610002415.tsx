...
export default function FeaturedProducts() {
  return (
    <section id="featured" className="bg-white px-3 py-12 sm:px-4 md:px-6 lg:px-10">
      ...
      <div className="mb-7 grid grid-cols-2 gap-2 font-body sm:mb-9 sm:grid-cols-2 sm:gap-3 lg:flex lg:flex-wrap lg:gap-2 lg:overflow-visible">
        {filterCategories.map((category, index) => (
          <button key={category} className={`font-body w-full rounded-full border px-3 py-2 text-[11px] text-center sm:px-4 sm:text-xs lg:w-auto lg:shrink-0 lg:px-5 ${index === 0 ? 'border-[#111111] bg-[#111111] text-white' : 'border-[#dedede] bg-white text-[#111111]'}`}>
            {category}
          </button>
        ))}
      </div>
