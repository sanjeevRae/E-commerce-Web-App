...
export default function FeaturedProducts() {
  return (
    <section id="featured" className="bg-white px-3 py-12 sm:px-4 md:px-6 lg:px-10">
      ...
      <div className="mb-7 grid grid-cols-2 gap-2 font-body sm:mb-9 sm:flex sm:flex-wrap sm:gap-2 sm:overflow-visible">
        {filterCategories.map((category, index) => (
          <button key={category} className={`font-body w-full rounded-full border px-3 py-2 text-[11px] text-center sm:w-auto sm:shrink-0 sm:px-5 sm:text-xs ${index === 0 ? 'border-[#111111] bg-[#111111] text-white' : 'border-[#dedede] bg-white text-[#111111]'}`}>
            {category}
          </button>
        ))}
      </div>
