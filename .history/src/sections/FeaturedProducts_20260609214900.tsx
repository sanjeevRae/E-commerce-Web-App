      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-5 lg:grid-cols-4">
        {products.slice(0, 4).map(product => (
          <Link key={product.id} href={`/product/${product.id}`} className="group min-w-0 text-left">
            ...
            <p className="mb-1 truncate text-[9px] uppercase tracking-[0.08em] text-[#777777] sm:text-[11px] sm:tracking-[0.1em]">{product.category}</p>
            <h3 className="line-clamp-2 min-h-[34px] text-[11px] font-medium leading-snug text-[#111111] sm:min-h-[42px] sm:text-sm">{product.name}</h3>
            <p className="mt-1.5 text-[12px] font-semibold text-[#111111] sm:mt-3 sm:text-sm">${product.price.toFixed(2)}</p>
          </Link>
        ))}
      </div>
