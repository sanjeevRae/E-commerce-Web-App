...
export default function Categories() {
  ...
  {categories.map(category => (
    ...
    >
      {/* Image covers the whole article */}
      <img
        src={category.image}
        alt={category.name}
      ...
      {/* Gradient and text combined – only at the bottom */}
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent px-2.5 pb-3 pt-6 text-center text-white sm:px-5 sm:pb-6 sm:pt-8">
        <h3 className="font-body mx-auto max-w-full text-sm font-medium leading-tight sm:max-w-[90%] sm:text-xl sm:font-normal md:text-2xl">
          {category.name}
        </h3>
        <p className="mt-1 text-[11px] leading-none text-white/90 sm:text-sm">
