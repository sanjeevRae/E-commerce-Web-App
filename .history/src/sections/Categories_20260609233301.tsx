    ...
    <section id="categories" className="bg-white px-3 pb-14 sm:px-4 sm:pb-20 md:px-4">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:gap-4 md:grid-cols-2">
        {categories.map(category => (
          <article key={category.name} className="group relative aspect-square overflow-hidden rounded-2xl bg-[#f2f2f2]">
            ...
            <div className="absolute inset-x-0 bottom-0 rounded-b-2xl bg-white/92 px-3 py-2.5 backdrop-blur-[2px] sm:px-5 sm:py-4">
              <div className="flex items-end justify-between gap-2">
                <div className="min-w-0">
                  <h3 className="font-body truncate text-sm font-medium leading-tight text-[#111111] sm:text-xl sm:font-normal md:text-2xl">{category.name}</h3>
                  <p className="mt-1 text-[11px] leading-none text-[#666666] sm:text-sm">{category.stock} Stock</p>
                </div>
                <ArrowUpRight className="mb-0.5 h-4 w-4 shrink-0 text-[#111111] sm:h-5 sm:w-5" strokeWidth={1.7} />
              </div>
            </div>
          </article>
        ))}
