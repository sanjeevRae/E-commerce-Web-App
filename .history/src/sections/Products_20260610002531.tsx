...
export default function Products() {
  ...
  const getMobileSlotStyle = (offset: number) => {
    const slots: Record<number, { left: string; opacity: number; scale: number; zIndex: number; pointerEvents: 'auto' | 'none' }> = {
      [-2]: { left: '4%', opacity: 0, scale: 0.72, zIndex: 1, pointerEvents: 'none' },
      [-1]: { left: '23%', opacity: 0.32, scale: 0.74, zIndex: 4, pointerEvents: 'auto' },
      [0]: { left: '50%', opacity: 1, scale: 1, zIndex: 20, pointerEvents: 'none' },
      [1]: { left: '77%', opacity: 0.32, scale: 0.74, zIndex: 4, pointerEvents: 'auto' },
      [2]: { left: '96%', opacity: 0, scale: 0.72, zIndex: 1, pointerEvents: 'none' },
    };
    return slots[offset];
  };
  ...
  return (
    <section id="products" className="overflow-hidden bg-white px-4 py-16 text-center md:px-6 md:py-20 lg:px-10 lg:py-14">
      ...
      <div className="relative left-1/2 h-[360px] w-screen max-w-[430px] -translate-x-1/2 sm:h-[390px] lg:hidden">
          ...
          return (
            ...
            >
              <img
                src={product.image}
                alt={product.name}
                className={`object-contain object-bottom transition-[height,width,filter,transform] duration-700 ${activeCard ? 'h-[340px] w-[58vw] max-w-[220px] sm:h-[370px]' : 'h-[220px] w-[31vw] max-w-[120px] sm:h-[245px]'}`}
