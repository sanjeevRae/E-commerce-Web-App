...
export default function Categories() {
  ...
  {categories.map(category => (
    ...
    >
      {/* Image covers the entire article */}
      <img
        src={category.image}
        alt={category.name}
      ...
      {/* Text directly on the image – no extra div, no gradient, no empty space */}
      <h3 className="absolute bottom-3 left-3 right-3 text-sm font-medium leading-tight text-white drop-shadow-lg sm:bottom-5 sm:left-5 sm:right-5 sm:text-xl md:text-2xl">
        {category.name}
      </h3>
      <p className="absolute bottom-3 left-3 text-[11px] leading-none text-white/90 drop-shadow sm:bottom-5 sm:left-5 sm:text-sm">
