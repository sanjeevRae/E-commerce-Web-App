    ...
    <section id="categories" className="bg-white px-3 pb-14 sm:px-4 sm:pb-20 md:px-4">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:gap-4 md:grid-cols-2">
        {categories.map(category => (
          <article key={category.name} className="group relative overflow-hidden rounded-2xl aspect-[3/4] sm:aspect-[16/10] sm:min-h-[230px] md:min-h-[280px]">
            <img src={category.image} alt={category.name} className="absolute inset-0 h-full w-full object-cover object-top transition-transform duration-500 group-hover:scale-105" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent" />
            <div className="absolute inset-x-0 bottom-0 px-2.5 pb-3 text-center text-white sm:px-5 sm:pb-6">
