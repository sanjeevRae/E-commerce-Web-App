    ...
    <section id="categories" className="bg-white px-3 pb-14 sm:px-4 sm:pb-20 md:px-4">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:gap-4 md:grid-cols-2">
        {categories.map(category => (
          <article key={category.name} className="group relative aspect-square overflow-hidden rounded-2xl bg-[#eeeeee] sm:aspect-[16/10] sm:min-h-[230px] md:min-h-[280px]">
            ...
            <div className="absolute inset-x-0 bottom-0 px-2.5 pb-3 text-center text-white sm:px-5 sm:pb-6">
              <h3 className="font-body mx-auto max-w-full text-sm font-medium leading-tight sm:max-w-[100%] sm:text-xl sm:font-normal md:text-2xl">{category.name}</h3>
